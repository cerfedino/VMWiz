...
func (s *postgresstorage) SurveyEmailGetAllNotAnsweredBySurveyID(surveyId int64) (*[]SQLUsageSurveyEmail, error) {
	...
	rows, err := s.db.Query(`SELECT id, recipient, surveyId, vmid, hostname, uuid, email_sent, still_used FROM survey_email WHERE surveyId=$1 AND still_used=null`, surveyId)
	if err != nil {
