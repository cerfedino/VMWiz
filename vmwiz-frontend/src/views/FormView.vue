    ...
    <div class="w-75 pa-6 ma-auto border-t-sm" style="max-width: 700px">
        ...
        <v-form ref="form">
            ...
            <v-text-field
                ...
                :rules="[
                    () => form_values.validation_errors.personalEmail || true,
                ]"
            />
          ...
          {{ form_values.tooltips.sshPubkey }}
            ...
            >
                <v-text-field
                    ...
                    :rules="[
                        () =>
                            form_values.validation_errors.sshPubkey[index] ||
                            true,
                    ]"
                >
                    <template v-slot:prepend>
                        <v-icon
                            ...
                            :disabled="
                                form_values.current.sshPubkey.length === 1
                            "
                            @click="
                                () => {
            ...
            <v-checkbox
                ...
                :rules="[
                    () => form_values.validation_errors.accept_terms || true,
                ]"
            >
...
export default {
    ...
    methods: {
        ...
        isFormModified() {
            return Object.keys(this.form_values.current).some((key) => {
                if (Array.isArray(this.form_values.current[key]))
                    ...
                    return (
                        this.form_values.current[key] !=
                        this.form_values.initial[key]
                    );
            });
        },
        ...
        storeFormState() {
            ...
            Object.keys(this.form_values.current).forEach((key) => {
                ...
                if (Array.isArray(currVal)) {
                    ...
                    if (
                        currVal.filter((x) => x != "" && !initVal.includes(x))
                            .length == 0
                    ) {
                        return;
                    } else {
        ...
        restoreFormState() {
            ...
            for (const [key, value] of Object.entries(query)) {
                if (!Object.keys(this.form_values.current).includes(key))
                    continue;

                if (value == "true") this.form_values.current[key] = true;
                else if (value == "false")
                    this.form_values.current[key] = false;
    ...
    mounted() {
        ...
        for (const [key, value] of Object.entries(this.form_values.current)) {
            if (Array.isArray(value))
                this.form_values.initial[key] = [...value];
